...
***** test
loc depvar capx_spend_aa_ann
reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 ///
...
loc depvar acquisitions_aa_ann
reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 `depvar'_l1 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol4_*  ///
		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
...
* note: relative to v1, I condition on not missing query_covfut_any_l1_l4, which is more relaxed that conditioning on query_covfut_any_l1_l1. Since it allows for any nonmissing values in the past 4 quarters.
...
foreach depvar in `depvarlist' {
	...
	* A. Baseline (Full sample)
	...
	if (`i'!=3) & (`i'!=4) {
		...
		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 `depvar'_l1 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol4_*  ///
		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
	}
...
// 	estadd scalar nviol = nviol
	...
	* B. Use anticipation as control 
	...
	if (`i'!=3) & (`i'!=4) {
		...
		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 `depvar'_l1 query_covfut_any_l1_l4 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol4_*  ///
		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
	}
...
// 	qui reghdfe `depvar'
// 	forval i = 1/3 {
// 		estimates store n`i'_5
// 		qui estadd scalar N = ., replace
// 		qui estadd scalar num_clusters = ., replace
// 		qui estadd scalar nviol = ., replace
// 	}
restore 


// esttab a*, keep(viol_confirmed_org)
// esttab b*, keep(viol_confirmed_org)
// esttab c*, keep(viol_confirmed_org)
// esttab d*, keep(viol_confirmed_org)
...
** Investment and employment proxies
	
* Panel A: Full sample
esttab a1_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
       keep(viol_confirmed_org) mtitles("$\Delta$ Log(Asset)" "$\Delta$ Log(PPE)" "$\Delta$ Capx/Asset" "$\Delta$ CashAcq/Asset" "$\Delta$ Employment") ///
...
esttab b1_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
...
esttab c1_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
esttab d1_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel A: Full sample
esttab a2_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
       keep(viol_confirmed_org) mtitles("$\Delta$ Log(Debt)" "$\Delta$  NDI/Assets" "$\Delta$ Log(Payout)" "$\Delta$ Cash/Asset" "$\Delta$ Log(DrawnRev)") nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel B: Anticipation as control
esttab b2_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel C: Mild bounding
esttab c2_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel D: Strict bounding
esttab d2_* using "$outdir/regression_violation_1_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
       append fragment posthead("\addlinespace \midrule  \multicolumn{3}{l}{\textit{Panel D: Exclude any anticipation}} & & & \\ ")
...
*** Operating variables
* Panel A: Full sample
esttab a3_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) mtitles("$\Delta$ CashFlow/Asset" "$\Delta$ Log(Sale)" "$\Delta$ Log(OpCost)" "1(Downgrade)" "1(Default)") ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
	   replace fragment posthead("\addlinespace \midrule  \multicolumn{3}{l}{\textit{Panel A: Full Sample}} & & & \\ ")
...
* Panel B: Anticipation as control
esttab b3_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel C: Mild bounding
esttab c3_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel D: Strict bounding
esttab d3_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
**** Access to credit variables
* Panel A: Full sample
esttab a4_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) mtitles("$\Delta$ Spread (TL)" "$\Delta$ Amount (TL)" "$\Delta$ Spread (CL)" "$\Delta$ Amount (CL)" "1\{Amend Terms\}") nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
	   append fragment posthead("\addlinespace \midrule  \multicolumn{3}{l}{\textit{Panel A: Full Sample}} & & & \\ ")
...
* Panel B: Anticipation as control
esttab b4_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel C: Mild bounding
esttab c4_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel D: Strict bounding
esttab d4_* using "$outdir/regression_violation_2_stacked_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
