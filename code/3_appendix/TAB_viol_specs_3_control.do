...
foreach depvar in `depvarlist' {
	...
	*-----------------------------------------------------------------
	* B. Use (binary) anticipation as control 
	...
	*-----------------------------------------------------------------
	* C. Use (continuous) anticipation as control	
	
	if (`i'!=3) & (`i'!=4) {
		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 query_covfut_wc_any_l1_l4 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol4_*  ///
		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
	}
	else {
		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 `depvar'_l1 query_covfut_wc_any_l1_l4 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol4_*  ///
		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
	}
	...
	*-----------------------------------------------------------------
	* D. Use (continuous - hi) anticipation as control
	
	if (`i'!=3) & (`i'!=4) {
		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 query_covfut_wc_any_hi_l1_l4 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol4_*  ///
		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
	}
	else {
		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 `depvar'_l1 query_covfut_wc_any_hi_l1_l4 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol4_*  ///
		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
	}
...
preserve 
	...
	* C. Use (continuous) anticipation as control	
	...
	qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l1 query_covfut_wc_any_l1_l1 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol1_*  ///
		if !missing(query_covfut_any_l1_l1), absorb(sic2 fyear) cluster(gvkey)
	...
	*-----------------------------------------------------------------
	* D. Use (continuous - hi) anticipation as control
	qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l1 query_covfut_wc_any_hi_l1_l1 ///
		opebitda_aa booklev intexpense_aa networth_seqq currentratio mkt_to_book ///
		ho2_* ho3_* hol1_*  ///
		if !missing(query_covfut_any_l1_l1), absorb(sic2 fyear) cluster(gvkey)
...
* Panel A: Full sample
esttab a1_* using "$outdir/regression_violation_1_control_may2025.tex", ///
...
esttab b1_* using "$outdir/regression_violation_1_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
       append fragment posthead("\addlinespace \midrule  \multicolumn{3}{l}{\textit{Panel B: Control for any anticipation (binary)}} & & & \\ ")

* Panel C: Mild bounding
esttab c1_* using "$outdir/regression_violation_1_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
       append fragment posthead("\addlinespace \midrule  \multicolumn{3}{l}{\textit{Panel C: Control for any anticipation (continuous)}} & & & \\ ")

* Panel D: Strict bounding
esttab d1_* using "$outdir/regression_violation_1_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
       append fragment posthead("\addlinespace \midrule  \multicolumn{3}{l}{\textit{Panel D: Control for intensive anticipation (binary)}} & & & \\ ")  postfoot("\addlinespace \midrule \addlinespace  \addlinespace")


** Financing policies

* Panel A: Full sample
esttab a2_* using "$outdir/regression_violation_1_control_may2025.tex", ///
...
* Panel B: Anticipation as control
esttab b2_* using "$outdir/regression_violation_1_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel C: Mild bounding
esttab c2_* using "$outdir/regression_violation_1_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel D: Strict bounding
esttab d2_* using "$outdir/regression_violation_1_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
       append fragment posthead("\addlinespace \midrule  \multicolumn{3}{l}{\textit{Panel D: Control for intensive anticipation (binary)}} & & & \\ ")
...
*** Operating variables
* Panel A: Full sample
esttab a3_* using "$outdir/regression_violation_2_control_may2025.tex", ///
...
* Panel B: Anticipation as control
esttab b3_* using "$outdir/regression_violation_2_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel C: Mild bounding
esttab c3_* using "$outdir/regression_violation_2_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel D: Strict bounding
esttab d3_* using "$outdir/regression_violation_2_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel A: Full sample
esttab a4_* using "$outdir/regression_violation_2_control_may2025.tex", ///
...
* Panel B: Anticipation as control
esttab b4_* using "$outdir/regression_violation_2_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel C: Mild bounding
esttab c4_* using "$outdir/regression_violation_2_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
* Panel D: Strict bounding
esttab d4_* using "$outdir/regression_violation_2_control_may2025.tex", ///
       keep(viol_confirmed_org) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
