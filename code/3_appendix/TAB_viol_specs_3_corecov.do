...
gen int_viol_corecov = 	viol_confirmed_org * core_covenant 
gen int_viol_l4_corecov = viol_confirmed_org_l4 * core_covenant
gen int_query_corecov = query_covfut_any_l1_l4 * core_covenant
...
label var int_viol_corecov "Violation x Core Covenant"
...
foreach depvar in `depvarlist' {
	...
	* A. Baseline (Full sample)
	...
	if (`i'!=3) & (`i'!=4) {
		...
		int_viol_corecov int_viol_l4_corecov ///
		i.core_covenant##( ///
	...
	qui sum viol_confirmed_org if ///
		!missing(query_covfut_any_l1_l4) & viol_confirmed_org==1 ///
...
//	
// 	if (`i'!=3) & (`i'!=4) {
// 		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l4 query_covfut_any_l1_l4 ///
// 		int_viol_corecov int_viol_l4_corecov int_query_corecov ///
// 		i.core_covenant##( ///
// 		c.opebitda_aa c.booklev c.intexpense_aa c.networth_seqq c.currentratio c.mkt_to_book ///
// 		c.ho2_* c.ho3_* c.hol4_*)  ///
// 		if !missing(query_covfut_any_l1_l4), absorb(sic2 datefq) cluster(gvkey)
...
//	
// 	qui sum viol_confirmed_org if ///
// 		query_covfut_wc_any_hi_l1_l4 == 0 & !missing(query_covfut_wc_any_hi_l1_l4) & viol_confirmed_org==1 ///
// 		& !missing(`depvar') & has_controls==1
// 	scalar nviol = `r(N)'
//	
// 	est store c1_`i'
...
// 	estadd scalar nviol = nviol	
	...
	* D. Drop sample where anticipation occurs in t-1 to t-4
	...
	qui sum viol_confirmed_org if query_covfut_any_l1_l4==0 & viol_confirmed_org==1 & !missing(`depvar') & has_controls==1
	scalar nviol = `r(N)'
...
preserve 
	...
	gen int_viol_corecov = 	viol_confirmed_org * core_covenant 
	gen int_viol_l1_corecov = viol_confirmed_org_l1 * core_covenant
	gen int_query_corecov = query_covfut_any_l1_l1 * core_covenant
	...
	foreach depvar in `depvarlist' {
		...
		* A. Baseline (Full sample)
		qui reghdfe `depvar' viol_confirmed_org viol_confirmed_org_l1 ///
			int_viol_corecov int_viol_l1_corecov ///
			i.core_covenant##( ///
			c.opebitda_aa c.booklev c.intexpense_aa c.networth_seqq c.currentratio c.mkt_to_book ///
			c.ho2_* c.ho3_* c.hol1_*) ///
			if !missing(query_covfut_any_l1_l1), absorb(sic2 fyear) cluster(gvkey)
			
		qui sum viol_confirmed_org if !missing(query_covfut_any_l1_l1) & viol_confirmed_org==1 & !missing(`depvar') & has_controls==1
		scalar nviol = `r(N)'	
...
//		
// 		scalar num_clusters = e(N_clust)
// 		qui sum viol_confirmed_org if !missing(query_covfut_any_l1_l1) & viol_confirmed_org==1 ///
// 			& !missing(`depvar') & has_controls==1
// 		scalar nviol = `r(N)'
//		
// 		est store b1_`i'
...
//		
// 		qui sum viol_confirmed_org if ///
// 			query_covfut_wc_any_hi_l1_l1==0 & !missing(query_covfut_wc_any_hi_l1_l1) & viol_confirmed_org==1 ///
// 			& !missing(`depvar') & has_controls==1
// 		scalar nviol = `r(N)'
//		
// 		est store c1_`i'
...
// 		estadd scalar nviol = nviol		
		...
		* D. Drop sample where anticipation occurs in t-1 (annual)
		...
		qui sum viol_confirmed_org if query_covfut_any_l1_l1==0 & viol_confirmed_org==1 & !missing(`depvar') & has_controls==1
		scalar nviol = `r(N)'
...
// esttab a*, keep(viol_confirmed_org int_viol_corecov)
// esttab b*, keep(viol_confirmed_org int_viol_corecov)
// esttab c*, keep(viol_confirmed_org int_viol_corecov)
// esttab d*, keep(viol_confirmed_org int_viol_corecov)
//
// * Panel A: Full sample
// esttab a* using "$outdir/regression_violation_3_corecov_may2025.tex", ///
//        keep(viol_confirmed_org int_viol_corecov) mtitles("$\Delta$ CashFlow/Asset" "$\Delta$ Log(Sale)" "$\Delta$ Log(OpCost)" "1(Downgrade)" "1(Default)") ///
//        se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
//        stats(N, fmt(0) layout(@) ///
//              labels("Observations")) ///
// 	   replace fragment posthead("\addlinespace \midrule \addlinespace \multicolumn{3}{l}{\textit{Panel A: Full Sample}} & & & \\ ")
//
// * Panel B: Anticipation as control
// esttab b* using "$outdir/regression_violation_3_corecov_may2025.tex", ///
//        keep(viol_confirmed_org int_viol_corecov) nomtitle nonum ///
//        se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
//        stats(N, fmt(0) layout(@) ///
//              labels("Observations")) ///
//        append fragment posthead("\addlinespace \midrule \addlinespace \multicolumn{3}{l}{\textit{Panel B: Anticipation as control}} & & & \\ ")
//
// * Panel C: Mild bounding
// esttab c* using "$outdir/regression_violation_3_corecov_may2025.tex", ///
//        keep(viol_confirmed_org int_viol_corecov) nomtitle nonum ///
//        se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
//        stats(N, fmt(0) layout(@) ///
//              labels("Observations")) ///
//        append fragment posthead("\addlinespace \midrule \addlinespace \multicolumn{3}{l}{\textit{Panel C: Intensive anticipation}} & & & \\ ")
//
// * Panel D: Strict bounding
// esttab d* using "$outdir/regression_violation_3_corecov_may2025.tex", ///
//        keep(viol_confirmed_org int_viol_corecov) nomtitle nonum ///
//        se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
//        stats(N, fmt(0) layout(@) ///
//              labels("Observations")) ///
//        append fragment posthead("\addlinespace \midrule \addlinespace \multicolumn{3}{l}{\textit{Panel D: Any anticipation}} & & & \\ ")
...
esttab a1_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) mtitles("$\Delta$ Log(Asset)" "$\Delta$ Log(PPE)" "Capx/Asset" "CashAcq/Asset" "$\Delta$ Employment") ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
	   replace fragment posthead(" \midrule  \multicolumn{3}{l}{\textit{Panel A: Full sample}} & & & \\ ")

* Panel B: Anticipation as control - investment activities 
esttab d1_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
       append fragment posthead(" \midrule  \multicolumn{3}{l}{\textit{Panel B: Exclude any anticipation}} & & & \\ ") postfoot(" \midrule ")
	   
* Panel C: Full sample - financing activity
esttab a2_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) mtitles("$\Delta$ Log(Debt)" "$\Delta$  NDI/Assets" "$\Delta$ Log(Payout)" "$\Delta$ Cash/Asset" "$\Delta$ Log(DrawnRev)") nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
	   append fragment posthead(" \midrule  \multicolumn{3}{l}{\textit{Panel A: Full sample}} & & & \\ ")

* Panel B: Anticipation as control financing activity 
esttab d2_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
esttab a3_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) mtitles("$\Delta$ CashFlow/Asset" "$\Delta$ Log(Sale)" "$\Delta$ Log(OpCost)" "1(Downgrade)" "1(Default)") nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
	   append fragment posthead(" \midrule  \multicolumn{3}{l}{\textit{Panel A: Full sample}} & & & \\ ")
...
esttab d3_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
...
esttab a4_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) mtitles("$\Delta$ Spread (TL)" "$\Delta$ Amount (TL)" "$\Delta$ Spread (CL)" "$\Delta$ Amount (CL)" "1\{Amend Terms\}") nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
	   append fragment posthead(" \midrule  \multicolumn{3}{l}{\textit{Panel A: Full sample}} & & & \\ ")
...
esttab d4_* using "$outdir/regression_violation_0_corecov_may2025.tex", ///
       keep(viol_confirmed_org int_viol_corecov) nomtitle nonum ///
       se(3) b(3) label compress starlevels(* 0.10 ** 0.05 *** 0.01) ///
       stats(N, fmt(0) layout(@) ///
             labels("Observations")) ///
       append fragment posthead(" \midrule  \multicolumn{3}{l}{\textit{Panel B: Exclude any anticipation}} & & & \\ ")
